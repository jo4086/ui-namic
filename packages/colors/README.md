...
-   ✅ **offset** 옵션 추가
    ...
    -   9단계 스펙트럼에서 기준 명도 토큰(기존 `500`)을 중심이 아닌 다른 위치로 이동 가능하게 합니다.

    -   예: `offset: 2` → `700`이 중심이 되고, `100~600`은 낮은 쪽, `800~900`은 높은 쪽으로 분포

    -   색상 중심값(`centerL`)은 그대로 유지한 채, **토큰 분포만 비틀어 특정 톤에 편향된 색상군**을 만들 수 있습니다.

    -   밝은 tone 중심 / 어두운 tone 중심 등 다양한 감성 테마 구현 가능
